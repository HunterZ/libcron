...
system_clock::time_point DT(year_month_day ymd,
                            hours          h = hours{0},
                            minutes        m = minutes{0},
                            seconds        s = seconds{0})
{
          ...
          const std::vector<system_clock::time_point>& expected_next)
{
  ...
  for (const auto& e : expected_next)
  {
    const auto& result{sched.calculate_from(curr_from)};
    const auto& calculated{std::get<1>(result)};

    if (!std::get<0>(result) || calculated != e)
    {
...
bool test(const std::string&       schedule,
          system_clock::time_point from,
          system_clock::time_point expected_next)
{
  auto c{CronData::create(schedule)};
  if (!c) { return false; }

  CronSchedule sched(*c);
  const auto&  result{sched.calculate_from(from)};
  const auto&  run_time{std::get<1>(result)};

  if (!std::get<0>(result) || expected_next != run_time)
  {
    std::cout << "From:       " << from << "\n"
...
{
  REQUIRE(
    test("0 0 * * * ?", DT(2010_y / 1 / 1), DT(2010_y / 1 / 1, hours{0})));
  REQUIRE(test("0 0 * * * ?",
               DT(2010_y / 1 / 1, hours{0}, minutes{0}, seconds{1}),
               DT(2010_y / 1 / 1, hours{1})));
  REQUIRE(test(
    "0 0 * * * ?", DT(2010_y / 1 / 1, hours{5}), DT(2010_y / 1 / 1, hours{5})));
  REQUIRE(test("0 0 * * * ?",
               DT(2010_y / 1 / 1, hours{5}, minutes{1}),
               DT(2010_y / 1 / 1, hours{6})));
  REQUIRE(test("0 0 * * * ?",
               DT(2017_y / 12 / 31, hours{23}, minutes{59}, seconds{58}),
               DT(2018_y / 1 / 1, hours{0})));
  REQUIRE(test("0 0 10 * * ?",
               DT(2017_y / 12 / 31, hours{9}, minutes{59}, seconds{58}),
               DT(2017_y / 12 / 31, hours{10})));
  REQUIRE(test("0 0 10 * * ?",
               DT(2017_y / 12 / 31, hours{23}, minutes{59}, seconds{58}),
               DT(2018_y / 1 / 1, hours{10})));
  REQUIRE(test("0 0 10 ? FEB *",
               DT(2017_y / 12 / 31, hours{23}, minutes{59}, seconds{58}),
               DT(2018_y / 2 / 1, hours{10})));
  REQUIRE(test("0 0 10 25 FEB ?",
               DT(2017_y / 12 / 31, hours{23}, minutes{59}, seconds{58}),
               DT(2018_y / 2 / 25, hours{10})));
  REQUIRE(test("0 0 10 ? FEB 1",
               DT(2017_y / 12 / 31, hours{23}, minutes{59}, seconds{58}),
               DT(year_month_day{2018_y / 2 / mon[1]}, hours{10})));
  REQUIRE(test("0 0 10 ? FEB 6",
               DT(2017_y / 12 / 31, hours{23}, minutes{59}, seconds{58}),
               DT(year_month_day{2018_y / 2 / sat[1]}, hours{10})));
  REQUIRE(test(
    "* * ? 10-12 NOV ?",
    DT(2018_y / 11 / 11, hours{10}, minutes{11}, seconds{12}),
    DT(year_month_day{2018_y / 11 / 11}, hours{10}, minutes{11}, seconds{12})));
  REQUIRE(
    test("0 0 * 31 APR,MAY ?", DT(2017_y / 6 / 1), DT(2018_y / may / 31)));
}
...
SCENARIO("Leap year")
{
  ...
  REQUIRE(test("0 0 * 29 FEB ?",
               DT(2020_y / 2 / 29, hours{15}, minutes{13}, seconds{13}),
...
SCENARIO("Multiple calculations")
{
  ...
  {
    REQUIRE(test("0 0/15 0/2 * * ?",
                 DT(2018_y / 1 / 1, hours{13}, minutes{14}, seconds{59}),
  ...
  {
    REQUIRE(test("0 0 0/12 12-13 JUL ?",
                 DT(2018_y / 1 / 1),
  ...
  {
    REQUIRE(test("0 0 0,12 1 */2 ?",
                 DT(2018_y / 3 / 10, hours{16}, minutes{51}),
                 DT(2018_y / 5 / 1)));
  }

  WHEN("“At 00:05 in August")
  {
    REQUIRE(test("0 5 0 * 8 ?",
                 DT(2018_y / 3 / 10, hours{16}, minutes{51}),
  ...
  {
    REQUIRE(test("0 0 22 ? * 1-5",
                 DT(2021_y / 12 / 15, hours{16}, minutes{51}),
...
SCENARIO("Examples from README.md")
{
  ...
  REQUIRE(test("0 0 12 * * MON-FRI",
               DT(2018_y / 03 / 10, hours{12}, minutes{13}, seconds{45}),
  ...
  REQUIRE(test("0 0 12 1/2 * ?",
               DT(2018_y / 01 / 2, hours{12}, minutes{13}, seconds{45}),
               {DT(2018_y / 1 / 3, hours{12}),
                DT(2018_y / 1 / 5, hours{12}),
                DT(2018_y / 1 / 7, hours{12})}));

  REQUIRE(test("0 0 */12 ? * *",
               DT(2018_y / 8 / 15, hours{13}, minutes{13}, seconds{45}),
