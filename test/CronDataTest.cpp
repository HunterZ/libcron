...
SCENARIO("Numerical inputs")
{
  GIVEN("Valid numerical inputs")
  {
    ...
    AND_WHEN("Using various ranges")
    {
      ...
      {
        REQUIRE(CronData::create("0-59 * * * * ?").has_value());
        REQUIRE(CronData::create("* 0-59 * * * ?").has_value());
        REQUIRE(CronData::create("* * 0-23 * * ?").has_value());
        REQUIRE(CronData::create("* * * 1-31 * ?").has_value());
        REQUIRE(CronData::create("* * * * 1-12 ?").has_value());
        REQUIRE(CronData::create("* * * ? * 0-6").has_value());
      }
    }
  }
  GIVEN("Invalid inputs")
  {
    WHEN("Creating items")
    {
      ...
      {
        REQUIRE_FALSE(CronData::create("").has_value());
        REQUIRE_FALSE(CronData::create("-").has_value());
        REQUIRE_FALSE(CronData::create("* ").has_value());
        REQUIRE_FALSE(CronData::create("* 0-60 * * * ?").has_value());
        REQUIRE_FALSE(CronData::create("* * 0-25 * * ?").has_value());
        REQUIRE_FALSE(CronData::create("* * * 1-32 * ?").has_value());
        REQUIRE_FALSE(CronData::create("* * * * 1-13 ?").has_value());
        REQUIRE_FALSE(CronData::create("* * * * * 0-7").has_value());
        REQUIRE_FALSE(CronData::create("* * * 0-31 * ?").has_value());
        REQUIRE_FALSE(CronData::create("* * * * 0-12 ?").has_value());
        REQUIRE_FALSE(CronData::create("60 * * * * ?").has_value());
        REQUIRE_FALSE(CronData::create("* 60 * * * ?").has_value());
        REQUIRE_FALSE(CronData::create("* * 25 * * ?").has_value());
        REQUIRE_FALSE(CronData::create("* * * 32 * ?").has_value());
        REQUIRE_FALSE(CronData::create("* * * * 13 ?").has_value());
        REQUIRE_FALSE(CronData::create("* * * ? * 7").has_value());
      }
    }
  }
}
  ...
  GIVEN("Literal inputs")
  {
    ...
    AND_WHEN("Using backward range")
    {
      ...
      AND_THEN("Range is valid")
      {
        ...
        REQUIRE(has_value_range(
          c->get_day_of_week(), 0, 3));  // Has sun, mon, tue, wed
        REQUIRE_FALSE(CronData::has_any_in_range(
          c->get_day_of_week(), 4, 5));  // Does not have thu or fri.
      }
    }
  }
}
...
{
  REQUIRE_FALSE(CronData::create("0 0 * 30 FEB *").has_value());
  REQUIRE_FALSE(CronData::create("0 0 * 31 APR *").has_value());
}

SCENARIO("Date that exist in one of the months")
{
  REQUIRE(CronData::create("0 0 * 31 APR,MAY ?").has_value());
}
...
{
  {
    ...
    REQUIRE(CronData::replace_string_name_with_numeric<libcron::DayOfWeek>(s) ==
            "0-2");
  }

  {
    std::string s = "JAN-DEC";
    REQUIRE(CronData::replace_string_name_with_numeric<libcron::Months>(s) ==
            "1-12");
  }
}
