...
std::chrono::system_clock::duration Task::time_until_expiry(
  std::chrono::system_clock::time_point now) const
{
  system_clock::duration d{};

  // Explicitly return 0s instead of a possibly negative duration when it has
  // expired.
  if (now >= next_schedule) { d = 0s; }
  else { d = next_schedule - now; }
...
std::string Task::get_status(std::chrono::system_clock::time_point now) const
{
  ...
  s += "' expires in ";
  s +=
    std::to_string(duration_cast<milliseconds>(time_until_expiry(now)).count());
  s += "ms => ";
