...
private:
  template<typename T>
  std::pair<bool, std::string> get_random_in_range(
  ...
  std::regex const   rand_expression{R"#([rR]\((\d+)\-(\d+)\))#",
...
template<typename T>
std::pair<bool, std::string> CronRandomization::get_random_in_range(
  const std::string& section, int& selected_value, std::pair<int, int> limit)
{
  ...
  if (std::regex_match(
        section.cbegin(), section.cend(), random_match, rand_expression))
  {
    ...
    if (limit.first != -1 && limit.second != -1)
    {
      ...
      {
        if (CronData::value_of(*it) < limit.first ||
            CronData::value_of(*it) > limit.second)
        {
          it = numbers.erase(it);
        }
        else { ++it; }
