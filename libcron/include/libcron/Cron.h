...
namespace libcron
{
    ...
    template<typename LockType>
    std::ostream& operator<<(std::ostream& stream, const Cron<LockType>& c);

    template<typename LockType = libcron::NullLock>
    class Cron
    {
        public:

            explicit Cron(std::shared_ptr<ICronClock> clock = std::make_shared<LocalClock>());
            ...
            friend std::ostream& operator<<<>(std::ostream& stream, const Cron<LockType>& c);
    ...
    template<typename LockType>
    Cron<LockType>::Cron(std::shared_ptr<ICronClock> clock)
        : clockSptr(clock)
    {
        if (!clock)
        {
    ...
    template<typename LockType>
    bool Cron<LockType>::add_schedule(std::string name, const std::string& schedule, Task::TaskFunction work)
    {
    ...
    std::tuple<bool, std::string, std::string>
    Cron<LockType>::add_schedule(const Schedules& name_schedule_map, Task::TaskFunction work)
    {
        bool is_valid = true;
    ...
    template<typename LockType>
    void Cron<LockType>::clear_schedules()
    {
        tasks.clear();
    }

    template<typename LockType>
    void Cron<LockType>::remove_schedule(const std::string& name)
    {
        tasks.remove(name);
    }

    template<typename LockType>
    std::chrono::system_clock::duration Cron<LockType>::time_until_next() const
    {
        return
        (
            tasks.empty() ?
    ...
    template<typename LockType>
    size_t Cron<LockType>::tick(std::chrono::system_clock::time_point now)
    {
        tasks.lock_queue();
        size_t res = 0;
    ...
    template<typename LockType>
    void Cron<LockType>::get_time_until_expiry_for_tasks(
        std::vector<std::tuple<std::string, std::chrono::system_clock::duration>>& status) const
    {
